...
export const updateRolService = async (req) => {
...
export const getPerfilService = async (req) => {
    const userId = req.user?.id_usuario;
    return await prisma.perfil.findUnique({ where: { id_usuario: userId } });
};

export const updatePerfilService = async (req) => {
    const userId = req.user?.id_usuario;
    if (!userId) {
...
export const getDashboardService = async (req) => {
    const user = req.user;
    if (!user) {
    ...
    let contenido = { mensaje: "", acciones: [] };

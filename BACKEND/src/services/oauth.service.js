...
export const getOAuthRedirectUrl = (provider) => {
    if (provider === "google") {
        if (!googleClient?.authorization_endpoint) {
    ...
    if (provider === "microsoft") {
        if (!microsoftClient?.authorization_endpoint) {
...
export const handleOAuthLoginService = async (req, res, provider) => {
    ...
    try {
        ...
        if (provider === "google") {
            ...
            nombre = userinfoRes.data.name || userinfoRes.data.given_name ||
                "Sin nombre";
        ...
        if (provider === "github") {
            ...
            userinfoRes = await axios.get(
                githubClientConfig.userinfo_endpoint,
                {
                    headers: { Authorization: `Bearer ${accessToken}` },
                },
            );
            ...
            const primaryEmail = emailsRes.data.find((e) =>
                e.primary && e.verified
            )?.email;
            email = primaryEmail || userinfoRes.data.email || null;
            nombre = userinfoRes.data.name || userinfoRes.data.login ||
                "Sin nombre";
        ...
        res.status(500).json({
            error: `Error al procesar el login con ${provider}`,
        });
    }
};
